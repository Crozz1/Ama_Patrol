...
class LocatorViewModel : ViewModel() {
    ...
    private val locatorState: StateFlow<LocatorState> = _locatorState

    private val _eventFlow = MutableSharedFlow<LocatorEffect>()
    ...
    fun onAction(action: LocatorAction) {
        when (action) {
            is LocatorAction.EnteredLatitude -> {
                _locatorState.value = locatorState.value.copy(
                ...
                viewModelScope.launch {
                    ...
                    result.collect {
                        if (it.status == Result.Status.SUCCESS) {
                            ...
                            _eventFlow.emit(
                                LocatorEffect.DrawPolygon(action.endLocation, shape = shape)
                            )
                        }
                    }
                }
            }
            is LocatorAction.SavingMode -> {}
        }
    }
