package com.ama_patrol.presentation.menu


import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.ama_patrol.data.network.datasource.map.MapDataSource
import com.ama_patrol.data.repository.map.MapRepository
import com.ama_patrol.data.repository.map.MapRepositoryImpl
import com.ama_patrol.data.network.Result
import kotlinx.coroutines.flow.MutableSharedFlow
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.flow.asSharedFlow
import kotlinx.coroutines.launch

class HomeViewModel : ViewModel() {

    //private val noteRepository: NoteRepository

    private val mapRepository: MapRepository

    private val _monitoringState = MutableStateFlow(
        MonitoringNoteState()
    )
    val monitoringState: StateFlow<MonitoringNoteState> = _monitoringState

    private val _eventFlow = MutableSharedFlow<HomeEffect>()
    val eventFlow = _eventFlow.asSharedFlow()

    init {
//        val noteDao = NoteDatabase.getDatabase(application).noteDao
//        noteRepository = NoteRepositoryImpl(noteDao)

        mapRepository = MapRepositoryImpl(MapDataSource())
    }

    fun onAction(action: MonitoringNoteAction) {
        when (action) {
            is MonitoringNoteAction.EnteredId -> {
                _monitoringState.value = monitoringState.value.copy(
                    id = action.value
                )
            }
            is MonitoringNoteAction.ChangeColor -> {
                _monitoringState.value = monitoringState.value.copy(
                    color = action.color
                )
            }
            is MonitoringNoteAction.EnteredDestination -> {
                _monitoringState.value = monitoringState.value.copy(
                    destination = action.value
                )
            }
            is MonitoringNoteAction.EnteredOrigin -> {
                _monitoringState.value = monitoringState.value.copy(
                    origin = action.value
                )
            }
            is MonitoringNoteAction.SavingMode -> {
                _monitoringState.value = monitoringState.value.copy(
                    saveMode = action.saveMode
                )
            }
            is MonitoringNoteAction.EnteredLatitude -> {
                _monitoringState.value = monitoringState.value.copy(
                    latitude = action.endLocation.latitude, longitude = action.endLocation.longitude
                )
                viewModelScope.launch {
                    val start =
                        action.startLocation.latitude.toString() + "," + action.startLocation.longitude.toString() //"-6.110116,106.931418"
                    val end =
                        action.endLocation.latitude.toString() + "," + action.endLocation.longitude.toString() //"-6.160455,106.905464"
                    val result = mapRepository.getDirection(
                        origin = start, destination = end,
                    )
                    result.collect {
                        if (it.status == Result.Status.SUCCESS) {
                            val shape = it.data?.routes?.get(0)?.overviewPolyline?.points.orEmpty()
                            _eventFlow.emit(
                                HomeEffect.DrawPolygon(action.endLocation, shape = shape)
                            )
                        }
                    }
                }
            }
        }
    }
}